...
class CollectionAPI {
  ...
  _getCollections(collectionPath) {
    return this._collections[collectionPath] || {};
  }

  _getCollectionOptions({ collectionPath }) {
    const { options } = this._getCollections(collectionPath);
    return options;
  }

  _getCollection({ collectionPath }) {
    const { collection } = this._getCollections(collectionPath);
    return collection;
  }
}
