...
class RequestListener {
  constructor(server, request, response) {
    ...
    const { collection } = this._server._collections[this._requestPath.collectionPath] || {};
    this._requestCollection = collection;
  ...
  _authenticate(...args) {
    const collectionOptions = this._server._collectionOptions(this._requestPath);
    let authCount = 0;
  ...
  _beforeHandling(method, ...args) {
    ...
    if (
      collectionOptions &&
      collectionOptions.before &&
      collectionOptions.before[method] &&
      isFunction(collectionOptions.before[method])
    ) {
      return collectionOptions.before[method].apply(this, args);
    }
  ...
  _afterHandling(method, ...args) {
    ...
    if (
      ...
      collectionOptions.after[method] &&
      isFunction(collectionOptions.after[method])
    ) {
      return collectionOptions.after[method].apply(this, args);
    }
